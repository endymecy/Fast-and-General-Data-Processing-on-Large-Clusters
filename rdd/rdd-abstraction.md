...
`RDDs`不需要总是被物化。相反，一个`RDD`拥有足够的关于其如何从其他数据集衍生（谱系）而可以从稳定存储的数据计算它的分区。这是一个强大的特性，本质上，
...
最后，为了说明我们的模型是怎么实现容错的，我们展示了图2.1中第三个查询的`RDDs`谱系图。在这个查询中，我们从错误开始，得出其所对应的`RDD`行，接着对行
...
特别是，`RDDs`不需要应付检查点的开销，因为它们可以使用谱系图而恢复。此外，只有`RDD`的丢失分区需要重新计算，而且它们可以在不同的节点上并行地重新计算，并不需要回滚整个程序。
...
正如介绍中所讨论，`RDDs`最适合在同一个数据集的所有元素上执行相同的操作的那些批处理应用程序。在这些情况下，`RDDs`可以高效的记住每一个转换操作，作为谱系图中的一个步骤，从而可以不需要记录大量的数据就
...
为了使用`Spark`，开发人员们写了一个把一个集群的工作进程连接起来的驱动程序，如图2.2所示那样。该驱动定义了一个或多个`RDDs`，并且调用它们的动作类。磁盘上的`Spark`代码也记录了`RDDs`的谱系图。
...
把`RDDs`作为抽象使用而面临的挑战之一是如何去选取一个可以在大量的`transform`之间追踪到其谱系图的表现形式。理想情况下，
...
要重新计算，并且它们可以并行地在不同节点上重新计算。而在宽依赖的谱系图中，单个失败的节点可能导致一个`RDD`的所有先祖`RDD`中的一些分区丢失，导致全部计算需要重新执行。
