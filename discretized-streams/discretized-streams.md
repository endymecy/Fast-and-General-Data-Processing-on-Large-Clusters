...
与曾经编写过离线（基于`Hadoop`）和在线处理应用的开发人员进行讨论，讨论结果显示这些特性具有重要的实用价值。与流式系统和批处理系统拥有各自单独 API 的系统相比，共享同一份代码库可以节省许多开发时间。
...
|特征|D-stream|连续处理系统|
|---|:---|:---:|
|延迟|0.5~2s|1-100ms ，除非为了一致性批次处理记录|
|一致性|记录在到达的时间间隔内原子性的进行处理|有些系统可以等待短暂的时间再继续执行同步操作【18，87】|
|记录延迟|延迟时间或应用级别的修正|延迟时间，乱序处理【67, 99】|
|故障恢复|快速并行恢复|在单节点上复制或串行恢复|
|慢任务恢复|通过推测执行|通常情况下没有处理|
|批处理/混合操作|通过`RDD API`的简单统一地处理|在一些数据库系统中存在【43】；在消息排队系统中不存在|


在介绍`D-Streams`的结尾部分，我们在表4.1.中将它的特征和连续处理系统进行了对比。它们的主要区别是`D-Streams`将任务划分成小且确定性的批处理任务。这会导致最小的延迟时间变长，但是可以让系统拥有更高效的恢复技术。
事实上，一些连续处理系统，比如`TimeStream`和`Borealis`【87, 18】, 同样也会将消息记录延迟，目的是为了执行那些具有多个上游消息流的确定操作（通过等待流中的周期性“断点”）以及确保系统的一致性，这导致这些系统的延迟时间由过去的毫秒级变为`D-Streams`里面的秒级。
